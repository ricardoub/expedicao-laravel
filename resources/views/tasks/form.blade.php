...
<div class="row">
  <div class="form-group col-md-6 col-sm-6">
    ...
    <div class="input-group">
      ...
      @if (!$options['form']['disabled'])
        {{ Form::select('percentage', $combos['percent10'], $model->percentage, ['class' => 'form-control', 'disabled' => $options['form']['disabled'] ]) }}
      @else
        {{ Form::text('percentage', $model->percentage, ['class' => 'form-control', 'placeholder' => 'Nome da opção', 'disabled' => $options['form']['disabled'] ]) }}
      @endif
    </div>
  </div>
